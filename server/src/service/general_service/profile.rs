...
async fn get_student_profile(user_data: TokenClaims, db: Arc<Mutex<Postgrest>>) -> GeneralResponse {
            let text_data = db.lock().await.from("student").select("student_id, full_name, birth, gender, address, email, phone").eq("student_id", user_data.user_id).execute().await.unwrap().text().await.unwrap();
...
async fn get_lecturer_profile(user_data: TokenClaims, db: Arc<Mutex<Postgrest>>) -> GeneralResponse {
            let text_data = db.lock().await.from("lecturer").select("lecturer_id, full_name, birth, gender, address, email, phone").eq("lecturer_id", user_data.user_id).execute().await.unwrap().text().await.unwrap();
...
async fn get_admin_profile(user_data: TokenClaims, db: Arc<Mutex<Postgrest>>) -> GeneralResponse {
            let text_data = db.lock().await.from("admin").select("admin_id, full_name, birth, gender, address, email, phone").eq("admin_id", user_data.user_id).execute().await.unwrap().text().await.unwrap();
