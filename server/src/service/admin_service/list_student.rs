...
use crate::model::{GeneralResponse, database_model::Student};
...
pub async fn list_student(
    ...
    Query(QueryOptions{page_number, students_per_page}): Query<QueryOptions>
    ...
    let student_query = db.lock().await
        .from("student")
        .select("student_id, full_name, birth, gender, address, email, phone").exact_count().range(from_index as usize, to_index as usize).execute().await.unwrap();
...
fn get_range_and_total(header: &HeaderMap) -> (String, u32) {
    let mut content_range = header.get("content-range").unwrap().to_str().unwrap().split("/");
